...
export default function AIPlanningScreen() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { setStepIndex((i) => (i < STEPS.length - 1 ? i + 1 : i)); }, 900);
    const run = async () => {
      try {
        const userId = await AsyncStorage.getItem("edumentor_user_id");
        const level = (await AsyncStorage.getItem("edumentor_level")) ?? "beginner";
        if (userId) { await generatePlan({ userId: userId as Id<"users">, level, cycleId: 1 }); }
    ...
    <View style={styles.container}>
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        ...
        <View style={styles.dotsRow}>
          {STEPS.map((_, i) => (<View key={i} style={[styles.dot, i <= stepIndex && styles.dotActive]} />))}
        </View>
...
function createStyles(c: typeof import("@/constants/theme").AppColors) {
  return StyleSheet.create({
    ...
    content: { alignItems: "center", gap: AppSpacing.lg, paddingHorizontal: AppSpacing.xl },
    iconWrap: { width: 120, height: 120, alignItems: "center", justifyContent: "center", position: "relative" },
    iconRing: {
      position: "absolute", width: 100, height: 100, borderRadius: 50,
      borderWidth: 2, borderColor: c.primaryLight, borderStyle: "dashed",
    },
    title: { fontSize: 28, fontWeight: "800", color: c.text, textAlign: "center", lineHeight: 36, letterSpacing: -0.5 },
    stepWrap: {
      backgroundColor: c.surface, borderRadius: Radius.full,
      paddingVertical: 10, paddingHorizontal: 20,
      shadowColor: c.primary, shadowOffset: { width: 0, height: 4 },
      shadowOpacity: 0.1, shadowRadius: 8, elevation: 3, minWidth: 260, alignItems: "center",
    },
    ...
    pill: { backgroundColor: c.primaryLight, borderRadius: Radius.full, paddingVertical: 6, paddingHorizontal: 14 },
